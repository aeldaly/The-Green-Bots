...
test -x $DAEMON || (echo "${DAEMON} can not be executed" && exit 0)

LOGDIR=$SUPERVISOR_ROOT/logs
PIDFILE=$SUPERVISOR_ROOT/$NAME.pid
...
DAEMON_OPTS="-c $SUPERVISOR_ROOT/conf/supervisord.conf $DAEMON_OPTS"
